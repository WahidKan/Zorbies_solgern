...
namespace Solgen.Formstack
{
    public class WebMergeClient : IWebMergeClient
    {
        ...
        public async Task<DataRoute> CreateDataRouteAsync(DataRouteRequest request)
        {
            var response = await httpClient.PostAsJsonAsync($"api/routes", request);
            response.EnsureSuccessStatusCode();
        ...
        public async Task<List<DataRoute>> GetDataRouteListAsync(string folder = null)
        {
        ...
        public async Task<List<Rule>> GetDataRouteRulesAsync(int dataRouteId)
        {
            var response = await httpClient.GetAsync($"api/routes/{dataRouteId}/rules");
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<List<Rule>>();
        }

        public async Task<DataRouteRequest> UpdateDataRoute(DataRouteRequest dataRouteRequest)
        {
            var response = await httpClient.PutAsJsonAsync($"api/routes/{dataRouteRequest.Id}", dataRouteRequest);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<DataRouteRequest>();
        }
        ...
        public async Task<List<InsuresignDeliveryRequest>> GetDocumentInsuresignDeliveries(int documentId)
        {
            var response = await httpClient.GetAsync($"api/documents/{documentId}/deliveries");
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<List<InsuresignDeliveryRequest>>();
        }

        public async Task<List<InsuresignDeliveryRequest>> GetRouteInsuresignDeliveries(int routeId)
        {
            var response = await httpClient.GetAsync($"api/routes/{routeId}/deliveries");
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<List<InsuresignDeliveryRequest>>();
        }

        public async Task<InsuresignDeliveryRequest> CreateInsuresignDocumentDelivery(int documentId, InsuresignDeliveryRequest request)
        {
            var response = await httpClient.PostAsJsonAsync($"api/documents/{documentId}/deliveries", request);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<InsuresignDeliveryRequest>();
        }

        public async Task<InsuresignDeliveryRequest> CreateInsuresignRouteDelivery(int routeId, InsuresignDeliveryRequest request)
        {
            var response = await httpClient.PostAsJsonAsync($"api/routes/{routeId}/deliveries", request);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadAsAsync<InsuresignDeliveryRequest>();
        }
    }
