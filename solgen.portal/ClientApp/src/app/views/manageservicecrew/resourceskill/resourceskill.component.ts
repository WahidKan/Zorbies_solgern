...
import { Skillmodel, ServiceresourseService } from '../../serviceresource/serviceresourse.service';
...
export class ResourceskillComponent implements OnInit {
  ...
  getServiceResource(id) {
    this.commonService.getMasterSubModuleList(id, 'ServiceCrewddl').subscribe((result: any) => {
  ...
  getResourceSkill() {
    this.commonService.getMasterItemsList('ResourceSkill').subscribe((result: any) => {
      ...
      // console.log("this.lstResourceSkill", this.lstResourceSkill);
    })
  }
  show(id) {
    ;
  ...
  Save() {
    if (this.addForm.valid) {
      ...
      this.serviceresourseService.addeditServiceCrewSKill(this.appmodel).subscribe((result: any) => {
