...
import { FormService, FormControlModel, Container1, Widget, GroupControls, Group } from './form.service';
...
export class AddeditformComponent implements OnInit {
  ...
  @ViewChild('propertiesform', { static: false }) propertiesform: PropertiespopupformComponent;
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(
      params => {
        ...
        this.submodule = submodule.trim().toLowerCase().replace(' ', '');
        ...
        this.getcustomfielddata(module, submodule, this.formMasterId);

        if (!this.isCompanyTypeFinancialInstitute) {
          this.getCustomFiledListByModuleAndSubModule('form', Number(this.formMasterId));
        }
        else {
  ...
  filterCustomFieldData(){
    debugger;
    var addedData=this.customCompnentValues;
    var filterData=this.commonService.customFieldsList.data;
  }
  get SearchFieldName() { return this.manageForm.get('SearchFieldName') }

  getCustomFiledListByModuleAndSubModule(moduleName, formmasterid) {
    let modulecode = moduleName.trim().toLowerCase().replace(' ', '');
    // let submodulecode = submoduleName.trim().toLowerCase().replace(' ', '');

   
      this.loadSave = true;
    // this.commonService.GetCustomFieldsList(modulecode, submodulecode, '', '', '').subscribe((result: any) => {
      this.commonService.GetCustomFormFieldList(modulecode, formmasterid, '', '').pipe(take(1)).subscribe((result: any) => {
        if (result) {
          this.customCompnentValues = [];
    ...
    this.formService.GetFormFieldsList(id, sid, formMasterId, this.companyId).subscribe((result: any) => {
      ...
      if (result) { 
        ...
        if (this.formService.customFieldsList) {
          ...
          this.formName = this.droped[0].group_display_name;
        }
      ...
      item.id = index; item.display_order = index;
        ...
        this.formService.SaveLAyourData(this.group, this.moduleid, this.submoduleid, this.formMasterId).subscribe((result: any) => {
          if (result.statusCode == 200) {
            this.toaster.success('Form fields has been updated successfully.');
