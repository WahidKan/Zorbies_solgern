...
import { LoanApplicationReportModel, CommonService, validationModel } from '../../shared/common.service';
...
import { CommonIconPickerComponent } from '../../shared/common-icon-picker/common-icon-picker.component';


@Component({
  selector: 'app-addeditcustomreport',
  templateUrl: './addeditcustomreport.component.html',
  styleUrls: ['./addeditcustomreport.component.scss']
})
export class AddeditcustomreportComponent implements OnInit {
  @ViewChild('condition', { static: false }) condition: NgSelectComponent;
  //@ViewChild('iconCssClass', { static: false }) iconCssClass: CommonIconPickerComponent;
  @ViewChild('myStartCalendar', { static: false }) startCalendar: Calendar;
  @ViewChild('droppedView', { static: false }) DroppedView: ElementRef;
  @ViewChild('selectListView', { static: false }) selectListView: ElementRef;
  @Input() offset: number;
  ...
  moduleId: any;
  subModuleId: any;
  subModuleFieldsSelectedValue: any;
  ...
  //showExportToExcelBtn: boolean = false;
  isSubModuleSelected: boolean = false;
  pageTitle: any;
  onChangingSubModuleName: boolean = false;
  ...
  isIconPopupVisible: boolean = false;
  ...
  hideFilterButton() {
    ...
    if (this.customFieldList1 == null) {
      this.showreportData = false;
      this.columnheading=null
    }
    else {
      this.showreportData = true;
    }
    
    //this.showExportToExcelBtn = false;
  }

  iconClassReceiver(data) 
  {
    this.iconCssClass = data;
  }
  ...
  ngOnInit() {
    ...
    this.onChangingSubModuleName = false;
    this.islinkWithDashboard = false;
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        ...
        this.pageTitle = 'Edit Dynamic Report'
        this.hideShowFilterSection = true;
        this.showreportData = false;
        this.hideFilter = true;
        this.showFilter = true;
       ...
       this.saveLoad = true;
        ...
        this.showreportData = true;
        this.showEditBtns = true;
        this.pageTitle = 'Add Dynamic Report'
        this.addBarChartForm.reset();
        //this.showExportToExcelBtn = false;
        //this.hideShowFilterSection = true;
      }
    })
  ...
  getSubModuleList() {
    this.commonService.GetMasterItems("SubModuleList").subscribe((result: any) => {
  ...
  resetSubModule() {
    ...
    this.customFieldList1 = null;
    this.FieldlistFilter = null;
    this.subModuleSelectedValue = null;
    this.subModuleFieldsSelectedValue = null;
    this.customFieldLeadListSecond = null;
    this.droped = [];
  };

  resetSubModuleFieldIds() {
    this.subModuleFieldsSelectedValue = null;
  };

  onSelectSubModule(event) {
    debugger
    this.saveLoad = true;
    this.reportForm.reset();
    this.customFieldList1 = null;
    ...
    this.subModuleFieldsSelectedValue = null;
    this.moduleId = event.masterName;
    this.subModuleId = (event.value);
    this.subModuleCode = event.name;
    this.subModuleTableName = event.email;
    this.onChangingSubModuleName = true;
    this.getSubModuleFields();
    this.droped = [];
    this.isSubModuleSelected = false;
  };

  getSubModuleFields() {
    debugger
    this.saveLoad = true;
    this.ruleEngineService.GetDynamicReportFieldsListBySubModuleId(this.moduleId, this.subModuleId, this.subModuleCode).subscribe((m: any) => {
      if (m) {
        ;
        ...
        ;
        if (this.reportId != null && this.reportId != undefined && this.reportId != 0 && !this.onChangingSubModuleName) {
          ;
        ...
        if (this.onChangingSubModuleName) {
          this.saveLoad = false;
        }
  ...
  getCustomFieldFilterData() {
    this.ruleEngineService.GetDynamicReportFieldsListBySubModuleId(this.moduleId, this.subModuleId, this.subModuleCode).subscribe((m: any) => {
      if (m) {
        ...
        this.customFieldList1 = tableFields;
        this.customFieldLeadListDummy = tableFields;
  ...
  getReportDataById2(id) {
    ...
    this.loanapplicationsService.getReportDataById(id, this.companyType).subscribe((response: any) => {
      ...
      this.subModuleId = response[0].sub_module_id;
      this.subModuleTableName = response[0].subModuleTableName;
      this.isCountWidget = response[0].isCountWidget;
      this.isChartView = response[0].isChartView;
      this.isListView = response[0].isListView;
      this.chartType = response[0].chartType;
      if (this.isCountWidget == true) {
      ...
      this.Data.forEach(childObj => {
        if (childObj.operatorId == null && childObj.filedNameId == null) {   //&& childObj.subModuleFieldId == null
      ...
      if (this.orderData != null) {
        this.orderData.forEach(ObjOrder => {
          let form_field_id = null;
          if (ObjOrder.form_field_id != null) {
      ...
      if (this.isCountWidget) {
        this.showApplyToFields = true;
        this.addCountWidgetForm.patchValue({
          radiogroup: response[0].countWidgetApplyTo,
          selectedRole: (response[0].countWidgetRole_Ids != null && response[0].countWidgetRole_Ids != "") ? response[0].countWidgetRole_Ids.split(",") : null
        });
      }

      if (this.isChartView) {
        this.yAxisList = this.droped;
        //this.addBarChartForm.reset({ radiogroup: "Only For Me" });
        this.addBarChartForm.patchValue({
          radioChartBtn: response[0].chartViewApplyTo,
          chartSelectedRole: (response[0].chartViewRole_Ids != null && response[0].chartViewRole_Ids != "") ? response[0].chartViewRole_Ids.split(",") : null,
          yAxisId: (response[0].chartView_Y_Axis != null && response[0].chartView_Y_Axis != "") ? response[0].chartView_Y_Axis.toString() : null
        });

        if (this.chartType == 'HorizontalChart') {
          this.isHorizontalChart = true;
        }
      ...
      if (this.isListView) {
        ...
        this.addListViewForm.patchValue({
          listViewRadioBtn: response[0].listViewRadioBtn,
          listViewselectedRole: (response[0].listViewselectedRole != null && response[0].listViewselectedRole != "") ? response[0].listViewselectedRole.split(",") : null,
          listViewFields: (response[0].listViewFieldsId != null && response[0].listViewFieldsId != "") ? response[0].listViewFieldsId.split(",") : null
        })
      }
  ...
  addFields(e) {
    (this.fields.controls[this.fields.controls.length - 1] as FormGroup).patchValue({type: e});
    this.fields.push(this.buildFields());
    this.condition.clearModel();
  };

  removeFields(index: any) {
    const message = `Are you sure you want to remove this condition?`;
    this.dialogService.confirm('Remove Condition', message).subscribe(confirmed => {
      if (confirmed) {
        ...
        this.toaster.success(`Condition has been removed successfully.`);
      }
    })
  };
  ...
  get yAxisId() { return this.addBarChartForm.get('yAxisId'); }
  get listViewFields() { return this.addListViewForm.get('listViewFields'); }



  onChangeField($event, field: FormGroup, i: number) {
    if ($event !== undefined) {
      this.filterFieldName = $event.PrimaryTableColumn;
      ...
      this.custom_field_ids = $event.form_field_id;
      this.field_code = $event.dt_code

      this.getOperators($event.form_field_id, field);
      ...
      if ($event.dt_code == 'select') {
        this.DdlValues.splice(i, 1, $event.select_options)
      ...
      var mainData = this.reportForm.value.fields.filter((element, index) => {
        ...
        if (element.filedNameId != null) {
          var field = this.customFieldList1.filter(function (elementfield, index) {
            ...
            if (elementfield.form_field_id != undefined) {
              return (elementfield.form_field_id === element.filedNameId.form_field_id || elementfield.form_field_id == element.filedNameId);
            }
        ...
        if (element.type == 'Or') {
          if (element.fieldText == "date") {
            this.customWhereCondition = this.customWhereCondition + ' (convert(varchar,' + 'dateadd(HOUR, -8,' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' )' + ', 23) ' + operatorstring + ') ' + element.type + ' ';
          } else if (element.fieldText == "datetime") {
            this.customWhereCondition = this.customWhereCondition + ' (convert(varchar,' + 'dateadd(HOUR, -8,' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' )' + ', 121) ' + operatorstring + ') ' + element.type + '';
          } else {
            this.customWhereCondition = this.customWhereCondition + ' (' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' ' + operatorstring + ') ' + element.type + '';
          }
        }
          ...
          if (element.fieldText == "date") {
            this.customWhereCondition = this.customWhereCondition + ' (convert(varchar,' + 'dateadd(HOUR, -8,' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' )' + ', 23) ' + operatorstring + ') ';
          } else if (element.fieldText == "datetime") {
            this.customWhereCondition = this.customWhereCondition + ' (convert(varchar,' + 'dateadd(HOUR, -8,' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' )' + ', 121) ' + operatorstring + ') ';
          } else {
            this.customWhereCondition = this.customWhereCondition + ' (' + field[0].subModuleAlias + '.' + field[0].PrimaryTableColumn + ' ' + operatorstring + ' ) ';
          }
  ...
  thisMopnth() {
    ...
    if (this.droped.length == 0) {
      this.toaster.error('Please select atleast one field.');
    }
    else {
  ...
  updateFieldlistFilter(event) {
    if (this.FieldlistFilter === '') {
      ...
      for (var i = this.customFieldLeadListSecond.length - 1; i >= 0; i--) {// added to remove already selected 
        for (var j = 0; j < this.droped.length; j++) {
          if (this.droped[j].name == (this.customFieldLeadListSecond[i].name) && this.droped[j].table_name == (this.customFieldLeadListSecond[i].table_name)) {
            this.customFieldLeadListSecond.splice(i, 1);
            break; // add if item found it should break the for statement
          }
        }
    ...
    ;
    this.customFieldLeadListSecond = this.TempcustomFieldLeadListSecond.filter(s => s.name.toLowerCase().includes(this.FieldlistFilter.toLowerCase()));
    ...
    for (var i = 0; i < this.droped.length; i++) {
      if (this.droped[i].selected == true) {
        for (var j = 0; j < this.customFieldLeadListDummy.length; j++) {
          if (this.droped[i].PrimaryTableColumn == this.customFieldLeadListDummy[j].PrimaryTableColumn) {
  ...
  OnShiftAllFromDropToDrag() {
    ...
    for (var i = 0; i < this.droped.length; i++) {
      this.droped[i].selected = false;
      for (var j = 0; j < this.customFieldLeadListDummy.length; j++) {
        if (this.droped[i].display_name == this.customFieldLeadListDummy[j].display_name) {
          this.droped[i].selected = false;
          this.customFieldLeadListSecond.splice(j, 0, this.droped[i]);
          break;
        }
  ...
  SaveAllFields(val: string = '') {
    if (this.addCountWidgetForm.valid) {
      if (this.reportName == null || this.reportName == '') {
        this.isReportNameValid = true;
        return;
      }
      if (this.subModuleSelectedValue == null || this.subModuleSelectedValue == '') {
        this.isSubModuleSelected = true;
        return;
      }
      if (this.droped.length == 0) {
        this.toaster.error('Please select atleast one selected field.');
        return;
      }

      //==================================Validation for Count Widget =============================
      if (this.isCountWidget == true) {
        let ListselectedRole = this.addCountWidgetForm.get('selectedRole').value;
        if (ListselectedRole == null && this.addCountWidgetForm.value.radiogroup == 'Roles') {
      ...
      //===========================================================================================
      //==================================Validation for Chart View ===============================
      if (this.isChartView == true) {
        if (this.chartType == '' || this.chartType == null) {
          this.toaster.error('Please select atleast one chart for view.');
          return;
        }
        if (this.chartType != '' && this.addBarChartForm.value.yAxisId == null) {
          this.toaster.error('Please select y-axis value.');
          return;
        }
        let ChartSelectedRole = this.addBarChartForm.get('chartSelectedRole').value;
        if (ChartSelectedRole == null && this.addBarChartForm.value.radioChartBtn == 'Roles') {
          ...
          this.toaster.error('Please select roles for chart view.');
          return;
        }
        if (ChartSelectedRole != null) {
          if (ChartSelectedRole.length == 0) {
            this.addBarChartForm.get('chartSelectedRole').setValidators([Validators.required]);
            this.addBarChartForm.controls["chartSelectedRole"].updateValueAndValidity();
            this.toaster.error('Please select roles for chart view.');
            return;
      ...
      //================================ Validation for List view ================================
      if (this.isListView == true) {

        if (this.addListViewForm.value.listViewFields == null) {
          this.toaster.error('Please select list view fields.');
          return;
        }
        else {
          if (this.addListViewForm.value.listViewFields.length == 0) {
            this.toaster.error('Please select list view fields.');
            return;
             }
          }

          let ListViewselectedRole = this.addListViewForm.get('listViewselectedRole').value;
          if (ListViewselectedRole == null && this.addListViewForm.value.listViewRadioBtn == 'Roles') {
            ...
            this.toaster.error('Please select roles for list view.');
            return;
          }
          if (ListViewselectedRole != null) {
            if (ListViewselectedRole.length == 0) {
              this.addListViewForm.get('listViewselectedRole').setValidators([Validators.required]);
              this.addListViewForm.controls["listViewselectedRole"].updateValueAndValidity();
              this.toaster.error('Please select roles for list view.');
              return;
      ...
      if (this.reportForm.valid) {
        if (!this.isDateFilterValid && !this.isReportNameValid) {
          if (this.iserror != 0) {
            ...
            this.loanApplicationReportModel.isCountWidget = this.isCountWidget;
            if (this.isCountWidget == true) {
              this.loanApplicationReportModel.radiogroup = this.addCountWidgetForm.value.radiogroup;
              this.loanApplicationReportModel.Roles = (this.addCountWidgetForm.value.selectedRole != null) ? this.addCountWidgetForm.value.selectedRole.join(",") : null;
            }
            this.loanApplicationReportModel.isChartView = this.isChartView;
            if (this.isChartView == true) {
              this.loanApplicationReportModel.radioChartBtn = this.addBarChartForm.value.radioChartBtn;
              this.loanApplicationReportModel.chartSelectedRole = (this.addBarChartForm.value.chartSelectedRole != null) ? this.addBarChartForm.value.chartSelectedRole.join(",") : null;
              this.loanApplicationReportModel.chartViewYAxis = this.addBarChartForm.value.yAxisId;
              this.loanApplicationReportModel.chartType = this.chartType;
            }

            this.loanApplicationReportModel.isListView = this.isListView;

            if (this.isListView == true) {
              this.loanApplicationReportModel.listViewRadioBtn = this.addListViewForm.value.listViewRadioBtn;
              this.loanApplicationReportModel.listViewselectedRole = (this.addListViewForm.value.listViewselectedRole != null) ? this.addListViewForm.value.listViewselectedRole.join(",") : null;
                this.loanApplicationReportModel.listViewFields = this.addListViewForm.value.listViewFields.join(',');
              }
            
            this.loanApplicationReportModel.moduleId = this.moduleId;
            this.loanApplicationReportModel.subModuleId = this.subModuleId;
           ...
           // this.selecteddata = JSON.stringify(this.ListViewSelectedItems);
            ...
            this.loanApplicationReportModel.reportIconClass = this.iconCssClass;
            this.loanApplicationReportModel.reportIconSpanColorClass = 'icon-bg';
      ...
      for (const key of Object.keys(this.addCountWidgetForm.controls)) {
        if (this.addCountWidgetForm.controls[key].invalid) {
          const invalidControl = this.el.nativeElement.querySelector('[formcontrolname="' + key + '"]');
          invalidControl.focus();
          break;
        }
      }
      this.commonService.validateAllFormFields(this.addCountWidgetForm);
    }
  };
  ...
  runLoanApplicationReport() {
    ...
    if (this.reportForm.valid) {
      if (!this.isDateFilterValid) {
          this.hideShowFilterSection = false;
          this.showreportData = true;
         // this.showExportToExcelBtn = true;
        var obj = {};
        this.droped.forEach(x => obj[x.Field] = x.Value);
        this.selecteddata = this.droped
        this.filterData = JSON.stringify(this.fieldmodel);
        this.runLoanApplicationReportModel.selecteddata = JSON.stringify(this.selecteddata);
        this.runLoanApplicationReportModel.filterData = this.filterData;
        this.runLoanApplicationReportModel.sortColumn = this.sortColumn;
        this.runLoanApplicationReportModel.sortDir = this.sortDir;
        this.runLoanApplicationReportModel.currentPage = this.currentPage;
  ...
  //  this.selectedListViewItems[this.selectedListViewItems.indexOf(item)].selected = !item.selected;
  //  this.ListViewSelectedItems = this.selectedListViewItems.filter(x => x.selected == true);
  //};
  ...
  private fetchMore(dataList: any) {
    ...
    this.loanapplicationsService.GetValueDll('', this.len, this.searchText, this.filterFieldName).subscribe((data: any) => {
      //data.forEach(item => {
      //  this.arrayList.push(item.name,item.value)
      //})
      this.DdlValues[0] = this.DdlValues[0].concat(data);
    })
  };
  ...
  onKeyGetValue(e: any, dataList: any) {
    this.len = 0
    this.searchText = e.target.value;
    this.loanapplicationsService.GetValueDll('', this.len, this.searchText, this.filterFieldName).subscribe((data: any) => {
      this.DdlValues[0] = data;
    })
  };
