import { Component, OnInit, Input, ViewChild, Directive, ElementRef, ViewEncapsulation, ChangeDetectorRef, HostListener  } from '@angular/core';
import { UserDetails, CommonService, ModuleList, DailerParam } from '../../shared/common.service';
import { DashboardTopNotificationModel, DashboardCountsModel, DashboardService, DashboardTopLeaseModel } from '../dashboard.service';
import { NotificationdetailComponent } from '../../shared/notificationdetail/notificationdetail.component';
import { FormGroup } from '@angular/forms';
import { ToastrService } from 'ngx-toastr';
import { debounceTime, first } from 'rxjs/operators';
import { loansectionarray, LoanapplicationserviceService } from '../../loanapplication/loanapplicationservice.service';
import { Router } from '@angular/router';
import { any } from 'underscore';
import { ModalDirective } from 'ngx-bootstrap/modal';
import { StagemanagementComponent } from '../../shared/stagemanagement/stagemanagement.component';
import { DomSanitizer } from '@angular/platform-browser';
import { ProposallistService, JsonData, CheckboxData, MappingTempalte } from '../../proposal/proposallist.service';
...
export class CustomerdashboardComponent implements OnInit {
  ...
  fileuplist: any;
  AccountName: any;
  email = 'farhanafzal.5400@gmail.com'
  contractNo: any;
  ContractId: any;
  signedDate:any;
  ContarctStatus:any;
  ...
  constructor(private dashboardService: DashboardService, private commonService: CommonService, private toaster: ToastrService, private router: Router,
              private compp: StagemanagementComponent,private cdr: ChangeDetectorRef,
  ...
  GetContactDetailsById(opportunityId: string) {
    ...
    this.dashboardService.GetCompanyAccountData(opportunityId).subscribe((result: any) => {
      ...
      this.ownerImage = result[0].OwnerProfilePic;
       ...
       this.systemSize=result[0].systemSize;
        ...
        this.ContractId = result[0].ContractId;
       ...
       this.OwnerEmail = result[0].OwnerEmail;
        ...
        this.OwnerId = result[0].OwnerId;
       this.OwnerRefId = result[0].OwnerRefId;
       this.OwnerCompanyID = result[0].OwnerCompanyID;
     ...
     this.GetContractStatus(this.ContractId,'contract')
  ...
  GetDocumentHistoryListData(data: any) {
    if (data == 0) {
      this.toaster.error('No document is available to show')
    } else {
  ...
  GetUploadDocument() {
    this.loanapplicationservice.submoduleId = this.submoduleId;
  ...
  GetServiceAppoinment(appoinment:any){
    if(appoinment == 0){
      this.toaster.error('No service appointment is available to view.')
    }else{
  ...
  GetContractData(data:any){
   if( data == 0){
    ...
    this.contractModel.GetContractData(this.ContractId,'contract')
   }
 ...
 GetUnsignedDocument(opportunityId){
  ...
  GetUnSignedDocumentList(data: any) {
    if (data == 0) {
      this.toaster.error('No document is available to show')
    } else {
 ...
 headerSetup(user: any) {
  this.commonService.getLoggedInName.subscribe((userdetail: any) => {
    ...
    if (userdetail.id == null) {
      ...
      this.usercompanyLogo = userdetail.companyLogo;
...
onLogoError(event){
 ...
 //Do other stuff with the event.target
  ...
  onProjectError(event) {
    event.target.src = 'assets/images/no_image_available.jpg'
  }

  openDailer(phoneNumber: string) {
    if (phoneNumber) {
      let phoneNumbers = '+1' + phoneNumber;
      this.commonService.SetdialerNumber = new DailerParam(phoneNumbers, this.OwnerCompanyID, this.OwnerId, this.submoduleName, this.OwnerRefId);
    } else {
      this.toaster.error('Phone Number is not available');
    }
