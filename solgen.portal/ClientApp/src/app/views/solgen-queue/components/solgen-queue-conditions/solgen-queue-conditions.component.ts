...
import { SolgenQueueConditionCalculationsComponent } from '../solgen-queue-condition-calculations/solgen-queue-condition-calculations.component';

@Component({
  selector: 'app-solgen-queue-conditions',
  templateUrl: './solgen-queue-conditions.component.html',
  styleUrls: ['./solgen-queue-conditions.component.scss']
})
export class SolgenQueueConditionsComponent implements OnInit,OnChanges {
  SelectedValue: any[] = [];
 ...
 @ViewChild('resultCalculationModal', { static: false }) resultCalculationModal: SolgenQueueConditionCalculationsComponent;
  sqlConditionOperators = ['Or', 'And'];
  hideFieldList = true;
  textFieldListButton = " Show Field List";
  moduleAlias: string = '';

  constructor(private service: SolgenQueueService, private commonService: CommonService) { }
  ...
  initForm() {
    this.targetForm = this.service.buildTarget();
  }
  ...
  onConditionChange() {
    this.conditions.valueChanges.subscribe((condition: any[]) => {
      this.service.buildTargetConditionDatabaseString(condition,this.targetForm);
    });
  }
  ...
  addCondition(event) {
    let condition = this.service.buidCondition();
  ...
  CalculationModal(tableField: FormGroup) {
    this.resultCalculationModal.show(tableField, this.queueForm.get('moduleId').value.value, this.queueForm.get('subModuleId').value.sub_module_id, this.customFieldsList);
  }
  ...
  TargetConditions(index: number) {
    return (this.queueForm.get('targets') as FormArray).at(index).get('conditions') as FormArray;
  }

  saveCondition() {
    this.targetForm.get('isConditionAdded').setValue(true);
    (this.queueForm.get('targets') as FormArray).at(this.rowNo).patchValue({
