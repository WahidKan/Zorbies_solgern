...
@Component({
  ...
  templateUrl: './creditreporttracking.component.html',
  styleUrls: ['./creditreporttracking.component.scss']
})
export class CreditreporttrackingComponent implements OnInit {
  ...
  onChange($event) {
    this.loading = true;
    this.reportService.getCreditScoreTrackingReportList( this.listFilter,this.expFrom, this.expTo,  this.sortColumn, this.sortDir, 0, this.pageSizeValue, this.loginuserid).subscribe(response => {
  ...
  reset() {
    ...
    this.reportService.getCreditScoreTrackingReportList(this.listFilter, this.expFrom, this.expTo,  this.sortColumn, this.sortDir, 0, 10, this.loginuserid).subscribe(response => {
      this.pagedData = this.reportService.pagedData;
      this.loading = false;
    }, error => {
      this.loading = false;
  ...
  setPage($event) {
    this.loading = true;
    this.reportService.getCreditScoreTrackingReportList(this.listFilter, this.expFrom, this.expTo,  this.sortColumn, this.sortDir, $event.page - 1, this.pageSizeValue, this.loginuserid).subscribe(response => {
      this.pagedData = this.reportService.pagedData;
      this.loading = false;
    }, error => {
      this.loading = false;
  ...
  ExportTOExcel() {
    this.loading = true;
    this.rowsForExport = [];
    this.reportService.getCreditScoreTrackingReportList(this.listFilter, this.expFrom, this.expTo, this.sortColumn, this.sortDir, 0, this.totalPageSize, this.loginuserid).subscribe(response => {
      ...
      if (this.pagedDataForImport.data.length > 0) {
        ...
        this.commonService.ExportData(this.rowsForExport, "Excel", "CreditReportTracking",null);
      }
      this.loading = false;
    }, error => {
      this.loading = false;
  ...
  generatePdf() {
    ...
    this.reportService.getCreditScoreTrackingReportList(this.listFilter, this.expFrom, this.expTo,  this.sortColumn, this.sortDir, 0, this.totalPageSize, this.loginuserid).subscribe(response => {
      ...
      if (this.pagedDataForImport.data.length > 0) {
        ...
        this
        this.commonService.ExportData(this.rowsForExport, 'PDF', "creditreporttracking", null);
      } else { }
      this.loading = false;
    }, error => {
      this.loading = false;
