...
export class ProjectComponent implements OnInit {
  ...
  sortDir = 'desc';
  sortColumn = 'id';
  moduleName = 'settings'; submoduleName = 'projects';
  pagedData: any = {
    pager: {},
    data: []
  };
  ...
  load: boolean = false;
  actionarray: any[] = [];
  tableName = 'tblprojectnew';
  jsoncondition: any;

  placeholder: string = 'Search by Name'
  searchlabelName: string = 'Name'
  listLabelName: string = 'Projects'

  validationModel: validationModel = new validationModel();

  userinfodata: any;
  contentHeader: object;
  constructor(private projectSevice: ProjectService, private dialogService: ConfirmationDialogService,
    ...
    let add = this.modulePermission.find(m => m.privilegecode.toUpperCase() == 'PROJECTSADD');
    if (add == undefined) {
      add = "null";
    } else {
      this.isAdd = true;
    }
    let update = this.modulePermission.find(m => m.privilegecode.toUpperCase() == 'PROJECTSUPDATE');
    if (update == undefined) {
      update = "null";
    } else {
      this.isUpdate = true;
    ...
    let deletedata = this.modulePermission.find(m => m.privilegecode.toUpperCase() == 'PROJECTSDELETE');
  ...
  onDeleteTask(resId: any) {
    ...
    this.dialogService.confirm('Delete Loanterm', message).subscribe(confirmed => {
      this.projectSevice.onDeleteTask(resId, '').subscribe((res: any) => {
  ...
  ngOnInit() {
    ...
    this.actionarray =
      [
      { "name": "View", "click": "goToPage(row)", "route": "/projects/view/", "title": "View Service Resource", "iclass": "feather-eye text-info pr-2", "condition": "1 == 1" },
      { "name": "Edit", "click": "goToPage(row)", "route": "/projects/editProject/", "title": "Edit", "iclass": "feather-edit-2 text-success", "condition": "this.isUpdate" },
  ...
  add() {
    this.router.navigateByUrl('/projects/addProject');
  }
  ...
  refresh() {
    ...
    this.loading = true;
    this.projectSevice.GetprojectList(this.listFiltertext, this.searchUserType, this.sortColumn, this.sortDir, this.currentPage, this.pageSizeValue, this.loginuserid, this.custom_view_id, this.searchFilter, this.moduleName, this.submoduleName, this.companyId, this.myCheckbox)
      .subscribe(response => {
        this.loadSave = true;
        ;
        this.load = false;
  ...
  //      if (confirmed) {
  //        this.projectSevice.DeleteRecords(this.deleteId, 'tblServiceResource').subscribe(r => {
  ...
  Delete(row: any) {
    const message = `Are you sure you want to delete Project  "${row.Name}"?`;
    this.dialogService.confirm('Delete Project', message).subscribe(confirmed => {
      if (confirmed) {
        this.loadSave = true;
        this.projectSevice.DeleteRecords(row.Id, 'tblprojectnew').subscribe(r => {
          this.toaster.success(`Project "${row.Name}" has been deleted successfully`);
          this.loadSave = false;
          this.refresh();
        }, error => {
        });
